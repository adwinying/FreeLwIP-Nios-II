...
ip2str (char *buf, const T_IN4_ADDR *ipaddr)
{
	...
	if (buf == NULL) {
		syscall(GetResource(RES_IP2STR_BUFF_LOCK));
		buf = addr_sbuf[-- bix];
		if (bix <= 0)
			bix = NUM_IPV4ADDR_STR_BUFF;
		syscall(ReleaseResource(RES_IP2STR_BUFF_LOCK));
		}
                  ...
                  uint8_t proto, uint8_t ttl, ATR nbatr)
{
	...
	/* ネットワークバッファを獲得する。*/
	if ((error = get_net_buf_ex(nbuf, (uint_t)(IF_IP4_HDR_SIZE + align),
	                                  (uint_t)(IF_IP4_HDR_SIZE + maxlen), nbatr)) != E_OK)
		return error;
...
in_rtnewentry (uint8_t flags, uint32_t tmo)
{
	...
	if (frt == NULL) {
		...
		syscall(GetResource(RES_IN_ROUTING_TBL));
		for (ix = NUM_STATIC_ROUTE_ENTRY; ix < NUM_ROUTE_ENTRY; ix ++) {
			rt = &routing_tbl[ix];
		...
		if (frt == NULL)
			frt = srt;
		frt->flags = 0;
		syscall(ReleaseResource(RES_IN_ROUTING_TBL));
		}
