...
ER
tcp_wait_swbuf (T_TCP_CEP *cep)
{
	if (IS_PTR_DEFINED(cep->sbuf))
		return tcp_wait_swbuf_ncs(cep);
	else
		return tcp_wait_swbuf_cs(cep);
	}
...
ER
tcp_wait_swbuf_cs (T_TCP_CEP *cep)
{
	FLGPTN	flag;
	SYSTIM	before, after;
	ER	error;
	...
	while ((cep->flags & TCP_CEP_FLG_WBCS_MASK) != TCP_CEP_FLG_WBCS_FREE) {
		...
		if ((error = tinet_wai_flg(cep->snd_flgid, TCP_CEP_EVT_SWBUF_READY,
		                     TWF_ORW, &flag)) != E_OK)
			return error;
		syscall(tinet_clr_flg(cep->snd_flgid, (FLGPTN)(~TCP_CEP_EVT_SWBUF_READY)));
