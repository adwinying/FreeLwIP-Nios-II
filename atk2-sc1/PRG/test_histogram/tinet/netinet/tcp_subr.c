...
tcp_alloc_auto_port (T_TCP_CEP *cep)
{
	...
	while (true) {
		...
		if (portno != TCP_PORTANY) {
			...
			syscall(GetResource(RES_TCP_CEP));
			...
			if (portno != TCP_PORTANY) {
				...
				syscall(ReleaseResource(RES_TCP_CEP));
				return;
				}
			syscall(ReleaseResource(RES_TCP_CEP));
			}
...
tcp_close (T_TCP_CEP *cep)
{
	...
	if (cep->snd_nblk_tfn != TFN_TCP_UNDEF || cep->rcv_nblk_tfn != TFN_TCP_UNDEF) {
		 ...
		 * 送受信不可になったことを知らせる。
		 */
		syscall(tinet_set_flg(cep->snd_flgid, TCP_CEP_EVT_SWBUF_READY));
		syscall(tinet_set_flg(cep->rcv_flgid, TCP_CEP_EVT_RWBUF_READY));
...
tcp_get_segment (T_NET_BUF **nbuf, T_TCP_CEP *cep,
                 uint_t optlen, uint_t len, uint_t maxlen, ATR nbatr)
{
	ER		error;
	...
	if ((error = IN_GET_DATAGRAM(nbuf,
	                             ...
	                             &cep->dstaddr.ipaddr,
	                             &cep->myaddr.ipaddr,
	                             IPPROTO_TCP, IP_DEFTTL, nbatr)) != E_OK) {
...
ER
tcp_can_send_more (T_TCP_CEP *cep, FN fncd)
{
	ER	error;
...
ER
tcp_can_recv_more (ER *error, T_TCP_CEP *cep, FN fncd)
{
	/*
...
tcp_wait_rwbuf (T_TCP_CEP *cep)
{
	...
	if (cep->rwbuf_count == 0) {
		...
		while (cep->rwbuf_count == 0) {
			if ((error = tinet_wai_flg(cep->rcv_flgid, TCP_CEP_EVT_RWBUF_READY, TWF_ORW, &flag)) != E_OK) {
				return error;
				}
			syscall(tinet_clr_flg(cep->rcv_flgid, (FLGPTN)(~TCP_CEP_EVT_RWBUF_READY)));
...
tcp_write_raque (T_NET_BUF *input, T_TCP_CEP *cep, uint_t thoff, uint8_t flags)
{
	...
	if (len <= MAX_TCP_REALLOC_SIZE) {
		 ...
		 */
		if (get_net_buf(&new, (uint_t)len) != E_OK)
			new = NULL;
		}
	else
		new = NULL;
...
tcp_init_cep (T_TCP_CEP *cep)
{
	...
	cep->snd_ssthresh= MAX_TCP_WIN_SIZE;		/* 輻輳ウィンドサイズ(snd_cwnd)の制限値	*/
	...
	/* フラグを初期化する。*/
	syscall(tinet_set_flg(cep->snd_flgid, TCP_CEP_EVT_SWBUF_READY));
	syscall(tinet_clr_flg(cep->rcv_flgid, TCP_CEP_EVT_RWBUF_READY));
	}
...
tcp_output_trace (T_NET_BUF *output, T_TCP_CEP *cep)
{
	...
	syscall(GetResource(RES_TCP_TRACE));
	time = get_tim_utime() / 1000;
	iph  = GET_IP_HDR(output);
	...
	syscall(ReleaseResource(RES_TCP_TRACE));
	}
