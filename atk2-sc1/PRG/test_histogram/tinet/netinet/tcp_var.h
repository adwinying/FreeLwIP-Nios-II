...
typedef struct t_tcp4_rep {
	...
	ResourceType	resid_lock;	/* 受付口ロック		*/
...
#define TCP_WAIT_SWBUF(cep)			tcp_wait_swbuf_cs(cep)
...
#define TCP_WAIT_SWBUF(cep)			tcp_wait_swbuf(cep)
...
#define TCP_WAIT_SWBUF(cep)			tcp_wait_swbuf_ncs(cep)
...
extern ER tcp_can_send_more (T_TCP_CEP *cep, FN fncd);
extern ER tcp_can_recv_more (ER *error, T_TCP_CEP *cep, FN fncd);
...
extern ER tcp_wait_rwbuf (T_TCP_CEP *cep);
...
extern ER tcp_wait_swbuf (T_TCP_CEP *cep);
...
extern ER tcp_wait_swbuf_cs (T_TCP_CEP *cep);
...
extern ER tcp_wait_swbuf_ncs (T_TCP_CEP *cep);
